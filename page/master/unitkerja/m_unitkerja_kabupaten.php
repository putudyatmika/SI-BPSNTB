...
$db = new db();
$conn = $db -> connect();
$sql_unitkerja = $conn -> query("select * from m_unitkerja where unit_jenis='2' order by unit_jenis,unit_kode asc");
...
<table class="table table-hover table-striped table-condensed">
	...
	while ($r = $sql_unitkerja ->fetch_object()) {
		...
		if (($es3==0) and ($i != 1)) echo '<tr class="info"><td colspan="7"></td></tr>';
		echo '
		<tr>
			<td>'.$r->unit_kode.'</td>
			<td>'.$r->unit_nama.'</td>
