...
if ($_POST['submit_unit']) {
	...
	if ($cek>0) {
		 if ($unit_parent==NULL) {
			$sql_unit_save = $conn -> query("update m_unitkerja set unit_nama='$unit_nama',unit_diupdate_oleh='$created',unit_jenis='$unit_jenis' where unit_kode='$unit_kode'");
		 }
		 else {
			 $sql_unit_save = $conn -> query("update m_unitkerja set unit_nama='$unit_nama',unit_diupdate_oleh='$created',unit_jenis='$unit_jenis',unit_parent='$unit_parent' where unit_kode='$unit_kode'");
		 }
