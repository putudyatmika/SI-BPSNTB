...
if (isset($_POST['check'])== '') {
	...
	foreach ($_POST['check'] as $key => $ckp_t_item) {
		...
		if ($_POST['ckpt_status']=='draft') {
			$sql_ajukan= $conn->query("update ckp_t set ckp_t_status='1',ckp_t_edit='1' where ckp_t_id='$ckp_t_id'");
		...
		elseif ($_POST['ckpt_status']=='ajukan') {
			$sql_ajukan= $conn->query("update ckp_t set ckp_t_status='2',ckp_t_edit='2' where ckp_t_id='$ckp_t_id'");
